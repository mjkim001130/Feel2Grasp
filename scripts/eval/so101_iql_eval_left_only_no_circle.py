#!/usr/bin/env python
"""
SO101 Robot Evaluation with IQL Policy (Left Camera Only, NO CIRCLE DETECTION)

This version:
- Uses only the left camera (right camera is broken)
- Does NOT use circle detection in policy state (70D instead of 72D)
- Circle detection is removed from state, only used for visual debugging

Usage:
    python so101_iql_eval_left_only_no_circle.py
"""
...
# ======================== Configuration ========================
# IQL Checkpoint paths (UPDATE THIS after training 70D model)
IQL_CHECKPOINT_PATH = "IQL_checkpoints/iql_step_1000000_full_0_0.7.pt"  
...
SAVE_DATASET = False
DATASET_REPO_ID = "mjkim00/IQL_eval_no_circle"
...
def main():
    ...
    # ======================== NO Circle Detector - Just find camera keys ========================
    ...
    if left_key is None:
        log_say("Warning: Could not find left camera key.")
        ...
        print("NOTE: Circle detection is NOT used in policy state (70D model)")

    # ======================== Load IQL Policy (NO CIRCLE DETECTION) ========================
    log_say("Loading IQL policy (NO CIRCLE DETECTION - 70D)...")
    ...
    if DISPLAY_DATA:
        init_rerun(session_name="iql_evaluation_no_circle")
    ...
    log_say("NO CIRCLE DETECTION in policy state (70D model)")
    ...
    print("  Expected state: 64 (front encoding) + 6 (joints) = 70D (NO circle detection)")
    print("="*80 + "\n")
    ...
    try:
        ...
        while step < max_steps:
            ...
            for i, name in enumerate(joint_names):
                ...
                if name == "gripper":
                    next_pos = np.clip(next_pos, 0.0, 20.0)
            ...
            if (step + 1) % 25 == 0:  # Every second
                ...
                print(f"[{step+1:4d}/{max_steps}] FPS: {fps_current:.1f} (NO circle detection in state)")
